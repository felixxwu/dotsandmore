...
for (const keyString in config) {
    const key = keyString as keyof typeof config
    typeSafeSet(storeState, key, config[key]['value'])
    typeSafeSet(units,      key, config[key]['unit'])
}
